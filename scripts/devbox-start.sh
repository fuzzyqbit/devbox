...
echo "DCV desktop:         https://${PRIVATE_IP}:8443  (browser web client or native DCV client; reachable from VPC; requires your CIDR in allowed_web_cidrs)"
echo ""
echo "Off-VPC? Use './run devbox-port-forward' for code-server (:8080), or add your CIDR to var.allowed_web_cidrs. DCV (:8443) is direct connect within the allowed CIDR — no port-forward."
