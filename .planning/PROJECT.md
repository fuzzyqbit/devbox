...
**Active milestone:** v3.0 — Jupyter notebook + mise (phases start at 8).
...
## Current Milestone: v3.0 Jupyter + mise

**Goal:** Add a hardened JupyterLab service and the `mise` version manager to the baked devbox image, reusing the established secrets/service/security-group patterns.

**Target features:**
- JupyterLab runs as a systemd service on boot, with a per-build random password generated by the `secrets` Ansible role and published to SSM Parameter Store SecureString (same pattern as code-server / noVNC)
- A new CIDR-gated security-group port for Jupyter, governed by `var.allowed_web_cidrs` (consistent with :8080 / :6080)
- Operator UX: `./run secrets-show` reveals the Jupyter password; `./run devbox-port-forward` reaches it off-VPC; `./run status` surfaces the Jupyter URL
- `mise` binary installed in the AMI with shell activation for `ec2-user` — binary only, no committed `.mise.toml`, no change to the existing per-language Ansible layers (Python/Go/Rust/Java/Node)
